...
# Original: local_runs/recife_modelo_v12_extracao_final/fetch_rain_leadA_positives.py
...
for r in df.itertuples():
    ...
    rows.append({"point_id": r.point_id, "rain_max_24h_chirps": rm,
                 "rain_decay_index_api_chirps": api, "n_days_found": n})
    time.sleep(0.3)

rain_df = pd.DataFrame(rows)
print(rain_df)
merged = df.merge(rain_df, on="point_id", how="left")
