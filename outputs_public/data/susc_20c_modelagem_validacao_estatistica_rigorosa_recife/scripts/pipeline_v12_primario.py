...
# Original: local_runs/recife_modelo_v12_extracao_final/pipeline_v12_primario.py
...
print(f"primary n={len(primary)} ({(primary.rotulo==1).sum()} pos / {(primary.rotulo==0).sum()} neg)")
...
def predictive_auc(df, feature_cols, k=5, n_repeats=50, seed=SEED):
    d = df.dropna(subset=feature_cols).reset_index(drop=True)
    X = d[feature_cols].values; y = d["rotulo"].astype(int).values
    loo = LeaveOneOut()
    y_true, y_score = [], []
...
univ_df.to_csv(f"{V12}/primaria_v12_mannwhitney_univariado.csv", index=False)
...
firth_df.to_csv(f"{V12}/primaria_v12_firth_coefs_multivariado.csv", index=False)
...
boot_df.to_csv(f"{V12}/primaria_v12_coefs_bootstrap.csv", index=False)
...
with open(f"{V12}/primaria_v12_auc_preditivo.json", "w") as f:
