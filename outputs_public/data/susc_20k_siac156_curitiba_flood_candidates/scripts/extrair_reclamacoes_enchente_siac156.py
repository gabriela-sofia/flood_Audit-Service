...
Uso:
    python extrair_reclamacoes_enchente_siac156.py --csv arq1.csv arq2.csv \
...
class EventWindow:
    janela_id: str
    data_evento_ddmmaaaa: str  # formato do campo DataCriacao no CSV: DD/MM/AAAA
    expected_bairros: set[str] = field(default_factory=set)  # já normalizados


def load_events(path: Path | str) -> list[EventWindow]:
    """Lê CSV de janelas de evento: janela_id,data_evento_ddmmaaaa,bairros (';'-separado)."""
    events = []
        ...
        for row in csv.DictReader(f):
            ...
            events.append(
                EventWindow(
                    ...
                    data_evento_ddmmaaaa=row["data_evento_ddmmaaaa"].strip(),
...
def extract_candidates(csv_paths: list[Path | str], events: list[EventWindow]) -> list[dict]:
    ...
    by_date = {ev.data_evento_ddmmaaaa: ev for ev in events}
    ...
    for csv_path in csv_paths:
        with open(csv_path, encoding="utf-8-sig", newline="") as f:
            ...
            for row in reader:
                ...
                dedupe_key = (ev.janela_id, normalize(bairro), normalize(logradouro), data_criacao, normalize(assunto))
...
def write_candidates_csv(rows: list[dict], out_path: Path | str) -> None:
    ...
    fieldnames = [
        "janela_id", "data_criacao", "tipo", "orgao", "assunto", "subdivisao", "situacao",
        "logradouro", "bairro", "regional", "origem", "bairro_esperado", "arquivo_fonte",
    ]
...
def main(argv=None) -> int:
    ...
    for r in rows:
        by_window[r["janela_id"]] = by_window.get(r["janela_id"], 0) + 1
