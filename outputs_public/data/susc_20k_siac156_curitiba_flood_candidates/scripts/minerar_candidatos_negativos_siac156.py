...
`docs/metodologia_cientifica/revp_criterio_ponto_negativo_recife_e_replicacao_curitiba_petropolis.md`:
  1. presença de outro fenômeno real (categoria do `categorias_negativas_curitiba.py`, nunca
  ...
  3. coordenada auditável (geocodificação Nominatim strong/medium -- feita em etapa separada,
     `geocodificar_nominatim.py`, já validada nesta rodada)
...
da rodada anterior (--max-por-ano 40). Corrigido para usar `ano_fonte` (derivado do próprio
...
Uso:
    python minerar_candidatos_negativos_siac156.py --csv ano1.csv ano2.csv ... \
...
from categorias_negativas_curitiba import is_curitiba_negative_category  # noqa: E402
    ...
    with open(csv_path, encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            ...
            if not logradouro.strip() or not bairro.strip():
                continue
            if ano_fonte is None:
                ano_fonte = data_criacao[-4:] if len(data_criacao) >= 4 else ""
            candidates.append(
                {
                    "ano_fonte": ano_fonte,
                    ...
                    # agora e so o conteudo do registro (ano_fonte, nao o path), estavel entre
                    # maquinas e sessoes.
                    "rank_key": stable_rank(ano_fonte, logradouro, bairro, data_criacao),
                }
            )
...
def main(argv=None) -> int:
    ...
    fieldnames = ["ano_fonte", "data_criacao", "assunto", "subdivisao", "situacao", "logradouro", "bairro", "regional", "origem", "rank_key"]
    ...
    by_year = Counter(r["ano_fonte"] for r in all_rows)
