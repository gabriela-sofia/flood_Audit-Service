...
Existe porque o `montar_dataset_final.py` so monta positivos (rotulo=1); a etapa equivalente do
...
documentadas em `susc_20k2_k3_replica_completa_metodo_recife_report.md` secao 7.3:
  ...
  - `data_evento` = data real da ocorrencia nao-hidrologica do proprio registro, nunca sintetica.

Uniao com uma geracao anterior (`--existentes`): linhas ja presentes no dataset negativo
anterior sao carregadas VERBATIM (mesmo `ponto_id`, mesma proveniencia) e candidatos novos que
...
`minerar_candidatos_negativos_siac156.py` usa `stable_rank(str(csv_path), ...)`, que depende do
...
Uso:
    python montar_dataset_negativos_final.py --geocoded geocodificados.csv \
...
PROHIBITED_USE = ("Nao usar como escore, threshold ou variavel derivada do rotulo. Ausencia de "
...
def build(geocoded: list[dict], positives: list[tuple[float, float]],
    ...
    if existentes:
        for row in existentes:
            out.append(dict(row))
            vistos.add((row["logradouro"], row["bairro"], row["data_evento"]))
    ...
    for row in geocoded:
        ...
        out.append({
            "regiao": "Curitiba",
            "rotulo": 0,
            "ponto_id": negative_point_id(row["logradouro"], row["bairro"], row["data_criacao"]),
            "lat": lat,
            "lon": lon,
            ...
            "tipo_fonte_negativo": NEGATIVE_SOURCE_TYPE,
            "fenomeno_ocorrencia": (f"non-flood ({row['assunto']}/{row['subdivisao']}) - causally "
                                      "independent of rain, see categorias_negativas_curitiba.py"),
            "papel_fonte_dataset": "siac156_curitiba_negative_v1",
            "uso_permitido": ALLOWED_USE,
            "uso_proibido": PROHIBITED_USE,
        })
    ...
    resumo = {
        ...
        "total_final": len(out),
        "por_confianca": dict(Counter(r["nivel_confianca"] for r in out)),
        "por_ano": dict(sorted(Counter(str(r["ano_fonte"]) for r in out).items())),
