...
densidade de queixa -- proximo demais de um "escore/proxy derivado do rotulo" (proibido pelas
regras fixas do projeto: "nunca usar scores, thresholds, proxies ou variaveis derivadas do
rotulo como features", "o modelo NAO deve descobrir enchentes"). PU bagging nao inventa nenhuma
...
Uso:
    python pipeline_v20s_piloto_pu_bagging_curitiba.py
"""
...
def pu_bagging_temporal_holdout(df_train: pd.DataFrame, df_test: pd.DataFrame,
    ...
    escore = pu_bagging_fit_predict(Xtrs, ytr, Xtes, n_bags=n_bags, seed=seed)
    ...
    return {"n_treino": int(len(dtr)), "n_teste": int(len(dte)),
            ...
            "n_bags": n_bags, "auc_holdout_pu_bagging": round(float(auc), 4)}
...
def main(argv=None) -> int:
    ...
    d2.to_csv(out / f"{args.tag}_cv_blocos_espaciais_pu_bagging.csv", index=False)
    mean_auc = round(float(d2["auc_holdout_pu_bagging"].mean()), 4)
