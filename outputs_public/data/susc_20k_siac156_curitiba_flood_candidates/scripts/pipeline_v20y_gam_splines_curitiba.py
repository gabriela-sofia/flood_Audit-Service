"""SUSC-20Y -- GAM aditivo com B-splines por variavel, Curitiba.
...
tentativa natural: GAM (generalized additive modelo) com spline por variavel, que capturaria a
...
features), entao o efeito de cada variavel isolada e recuperavel diretamente (soma da base de
spline daquela variavel vezes seu proprio coeficiente), sem precisar de partial_dependence pra
...
par, cada variavel ganha uma curva de efeito flexivel (spline cubica, poucos nos), mas sem
...
recuperar o sinal, a nao-linearidade e por-variavel (boa noticia, interpretavel). Se nao
...
Uso:
    python pipeline_v20y_gam_splines_curitiba.py
"""
...
def fit_gam(df_train: pd.DataFrame, feature_cols: list[str], n_knots: int = DEFAULT_N_KNOTS,
    ...
    """Ajusta 1 SplineTransformer por variavel (independentes) + LogisticRegression aditiva
    ...
    for feat in feature_cols:
        ...
        parts.append(Xb)
    Xtr = np.hstack(parts)
    ytr = df_train["rotulo"].astype(int).values
...
def additive_effect_curves(splines: dict[str, SplineTransformer], clf: LogisticRegression,
    ...
    """Curva de efeito aditivo isolado por variavel: spline(x) . coef_dessa_feature.
    ...
    for feat, lo, hi in offsets:
        ...
        direcao = "crescente" if effect[-1] > effect[0] else "decrescente"
        rows.append({
            "variavel": feat, "grade_min": round(float(x_min), 4), "grade_max": round(float(x_max), 4),
            "efeito_no_min": round(float(effect[0]), 4), "efeito_no_max": round(float(effect[-1]), 4),
            "efeito_amplitude": round(float(effect.max() - effect.min()), 4),
            "monotonic": bool(n_sign_changes == 0), "n_trocas_sinal_inclinacao": n_sign_changes,
            "direcao": direcao, "sinal_esperado": EXPECTED_SIGN.get(feat),
            "direcao_bate_esperada": bool(
                (EXPECTED_SIGN.get(feat, 0) > 0 and direcao == "crescente") or
                (EXPECTED_SIGN.get(feat, 0) < 0 and direcao == "decrescente")),
        })
    return pd.DataFrame(rows)
...
def main(argv=None) -> int:
    ...
    grid.to_csv(out / f"{args.tag}_grade_hiperparam.csv", index=False)
    grid_summary = {
        "n_combos": int(len(grid)), "auc_min": round(float(grid["auc_holdout_2026"].min()), 4),
        "auc_median": round(float(grid["auc_holdout_2026"].median()), 4),
        "auc_max": round(float(grid["auc_holdout_2026"].max()), 4),
        "pct_above_gbm": round(float((grid["auc_holdout_2026"] >= REFERENCE_GBM_HOLDOUT_2026).mean() * 100), 1),
        "pct_above_linear_baseline": round(float((grid["auc_holdout_2026"] > REFERENCE_LINEAR_HOLDOUT_2026).mean() * 100), 1),
    }
    print(json.dumps(grid_summary, indent=2))

    print("[4] curvas de efeito aditivo por variavel (config default, ajustado no treino completo)")
    ...
    effects.to_csv(out / f"{args.tag}_curvas_efeito_aditivo.csv", index=False)
    print(effects.to_string(index=False))

    resumo = {
        "reference_linear_baseline": REFERENCE_LINEAR_HOLDOUT_2026,
        "reference_gbm_v20u": REFERENCE_GBM_HOLDOUT_2026,
        "gam_holdout_auc_2026_default_config": holdout["auc_holdout_2026"],
        ...
        "n_features_direction_matches_expected": int(effects["direcao_bate_esperada"].sum()),
