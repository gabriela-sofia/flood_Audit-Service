...
def medoid_indices(matrix: np.ndarray, labels: np.ndarray) -> dict[int, int]:
    result: dict[int, int] = {}
    for rotulo in sorted(set(int(x) for x in labels)):
        idx = np.where(labels == rotulo)[0]
        sub = matrix[idx]
        centroid = sub.mean(axis=0)
        distances = np.linalg.norm(sub - centroid, axis=1)
        result[rotulo] = int(idx[int(distances.argmin())])
    return result
...
def edge_indices(matrix: np.ndarray, labels: np.ndarray) -> dict[int, int]:
    ...
    for rotulo in sorted(set(int(x) for x in labels)):
        ...
        centroid = sub.mean(axis=0)
        distances = np.linalg.norm(sub - centroid, axis=1)
        result[rotulo] = int(idx[int(distances.argmax())])
    return result
...
def visual_review(rows: list[dict[str, str]], matrix: np.ndarray, ids: list[str], labels: np.ndarray, output_dir: Path, image_size: int, top_k: int) -> tuple[list[dict[str, object]], list[dict[str, object]], list[dict[str, object]], list[dict[str, object]]]:
    ...
    for source in ids:
        ...
        if make_mosaic(paths, mosaic, image_size):
            visual_manifest.append({"panel_type": "nearest_neighbors", "source_patch": by_id[source].get("patch_id", ""), "neighbor_patch": "|".join(by_id[n].get("patch_id", "") for n in neighbors if n in by_id), "cluster_id": int(labels[ids.index(source)]), "outlier_type": "", "image_path": str(mosaic), "embedding_distance": "", "regiao": by_id[source].get("regiao", "")})
    ...
    for cluster_id, idx in medoids.items():
        dino_id = ids[idx]
        medoid_rows.append({"cluster_id": cluster_id, "dino_input_id": dino_id, "patch_id": by_id[dino_id].get("patch_id", ""), "regiao": by_id[dino_id].get("regiao", ""), "medoid_status": "STRUCTURAL_REPRESENTATIVE_ONLY"})
        stable_rows.append({"representative_type": "cluster_medoid", "dino_input_id": dino_id, "patch_id": by_id[dino_id].get("patch_id", ""), "regiao": by_id[dino_id].get("regiao", ""), "stability_status": "SELECTED_SINGLE_RUN"})
        ...
        if dino_id in patch_images and make_mosaic([patch_images[dino_id]], image, image_size):
            visual_manifest.append({"panel_type": "cluster_medoid", "source_patch": by_id[dino_id].get("patch_id", ""), "neighbor_patch": "", "cluster_id": cluster_id, "outlier_type": "", "image_path": str(image), "embedding_distance": "0", "regiao": by_id[dino_id].get("regiao", "")})
        ...
        if edge_id in patch_images and make_mosaic([patch_images[edge_id]], edge_image, image_size):
            visual_manifest.append({"panel_type": "cluster_edge_case", "source_patch": by_id[edge_id].get("patch_id", ""), "neighbor_patch": "", "cluster_id": cluster_id, "outlier_type": "CLUSTER_EDGE", "image_path": str(edge_image), "embedding_distance": "", "regiao": by_id[edge_id].get("regiao", "")})

    for regiao in sorted({row.get("regiao", "") for row in rows}):
        region_idx = [i for i, row in enumerate(rows) if row.get("regiao") == regiao]
        centroid = matrix[region_idx].mean(axis=0)
        chosen = region_idx[int(np.linalg.norm(matrix[region_idx] - centroid, axis=1).argmin())]
        dino_id = ids[chosen]
        image = visual_dir / f"region_{safe_name(regiao)}_exemplar.png"
        if dino_id in patch_images and make_mosaic([patch_images[dino_id]], image, image_size):
            visual_manifest.append({"panel_type": "region_exemplar", "source_patch": by_id[dino_id].get("patch_id", ""), "neighbor_patch": "", "cluster_id": int(labels[chosen]), "outlier_type": "", "image_path": str(image), "embedding_distance": "", "regiao": regiao})
...
def spatial_metrics(rows: list[dict[str, str]], matrix: np.ndarray, ids: list[str], top_k: int) -> tuple[list[dict[str, object]], list[dict[str, object]], list[dict[str, object]]]:
    ...
    for i, row in enumerate(rows):
        order = [j for j in np.argsort(-sims[i]) if j != i]
        top = order[:top_k]
        same_region = [j for j in top if rows[j].get("regiao") == row.get("regiao")]
...
def multiscale(rows: list[dict[str, str]], matrix: np.ndarray) -> tuple[list[dict[str, object]], list[dict[str, object]]]:
    ...
    for regiao in sorted({row.get("regiao", "") for row in rows}):
        idx = [i for i, row in enumerate(rows) if row.get("regiao") == regiao]
        pooled_by_region[regiao] = normed[idx].mean(axis=0)
    sim_rows = []
    pool_rows = []
    for i, row in enumerate(rows):
        pooled = pooled_by_region[row.get("regiao", "")]
...
def outlier_taxonomy(rows: list[dict[str, str]], matrix: np.ndarray, labels: np.ndarray, density_rows: list[dict[str, object]]) -> tuple[list[dict[str, object]], list[dict[str, object]]]:
    ...
    for i, row in enumerate(rows):
        ...
        taxonomy.append({"dino_input_id": row["dino_input_id"], "outlier_categories": "|".join(categories), "nearest_distance": nearest[i], "cluster_id": int(labels[i]), "regiao": row.get("regiao", "")})
    overlap = []
...
def run(args: argparse.Namespace) -> int:
    ...
    summary = {
        ...
        "regions": sorted({row.get("regiao", "") for row in rows}),
    ...
    write_csv(output_dir / "visual_review_manifest.csv", visual_manifest, ["panel_type", "source_patch", "neighbor_patch", "cluster_id", "outlier_type", "image_path", "embedding_distance", "regiao"])
    ...
    write_csv(output_dir / "cluster_medoids.csv", medoids, ["cluster_id", "dino_input_id", "patch_id", "regiao", "medoid_status"])
    write_csv(output_dir / "stable_representatives.csv", stable_representatives, ["representative_type", "dino_input_id", "patch_id", "regiao", "stability_status"])
    write_csv(output_dir / "outlier_taxonomy.csv", outlier_rows, ["dino_input_id", "outlier_categories", "nearest_distance", "cluster_id", "regiao"])
