...
Uso:
    python revp_v1hd_reinterpretacao_com_imagem.py --sentinel-root /caminho/do/sentinel
    REVP_SENTINEL_ROOT=/caminho/do/sentinel python revp_v1hd_reinterpretacao_com_imagem.py
...
def _resolve_tif(patch_id: str, regiao: str, sentinel_root: Path) -> Path | None:
...
def stats_to_visual_notes(stats: ImageStats, patch_id: str, category: str) -> str:
    ...
    if b > 0.65:
        notas.append(f"Alta reflexão média ({b:.2f}) — possível solo exposto, superfície impermeável ou nebulosidade")
    elif b < 0.25:
        notas.append(f"Baixa reflexão média ({b:.2f}) — possível superfície escura, água aparente ou sombra")
    else:
        notas.append(f"Reflexão moderada ({b:.2f})")

    # Spatial heterogeneity (texture proxy)
    if std > 0.20:
        notas.append(f"Alta heterogeneidade visual (std={std:.2f}) — padrão texturizado, possível mistura de cobertura")
    elif std > 0.12:
        notas.append(f"Heterogeneidade moderada (std={std:.2f}) — variação espacial observável")
    else:
        notas.append(f"Padrão visual relativamente homogêneo (std={std:.2f})")
    ...
    if stats.blue_ratio > 0.37:
        notas.append("Elevada componente azul relativa — possível nebulosidade ou superfície aquática")

    if stats.green_ratio > stats.red_ratio + 0.03:
        notas.append("Canal verde dominante — padrão consistente com vegetação aparente")
    elif stats.red_ratio > stats.green_ratio + 0.03:
        notas.append("Canal vermelho dominante — padrão consistente com solo exposto ou área urbana aparente")
...
def stats_to_discussion_note(
    ...
    if patch_id in MEDOIDS:
        return (
            f"Medoid {regiao}: padrão estrutural central da região no espaço de embeddings. "
    ...
    if patch_id in OUTLIERS:
        return (
            f"Outlier {regiao}: patch periférico no espaço de embeddings. "
            f"Estatísticas visuais: NDVI médio={ndvi:.2f} ({veg:.0%} vegetação aparente), "
            f"heterogeneidade={stats.std_brightness:.2f}. "
    ...
    if patch_id in V1GU_CORPUS:
        return (
            f"Patch em corpus v1gu ({regiao}): baixa cobertura GIS + evidência estrutural disponível. "
    ...
    return (
        f"Patch sem evidência de embedding individual ({regiao}). "
...
def process_candidates(
    ...
    for rec in records:
        ...
        if rec.preview_status == "GENERATED" and sentinel_root is not None:
            tif_path = _resolve_tif(rec.canonical_patch_id, rec.regiao, sentinel_root)
        ...
        if stats is not None:
            ...
            discussion_note = stats_to_discussion_note(
                stats, rec.canonical_patch_id, rec.candidate_category, rec.regiao, uncertainty
            )
            structural_ctx = (
                f"v1gu corpus: {'yes' if rec.canonical_patch_id in V1GU_CORPUS else 'no'}; "
                f"papel: {'medoid' if rec.canonical_patch_id in MEDOIDS else ('outlier' if rec.canonical_patch_id in OUTLIERS else 'candidate')}; "
                f"regiao: {rec.regiao}"
            )
            ext_evidence = (
            ...
            discussion_note = (
                f"Visual evidence not computed ({rec.regiao} — {rec.candidate_category}). "
...
def build_synthesis(rows: list[dict], summary: dict) -> str:
    ...
    for r in medoid_rows:
        lines.append(f"**{r['review_item_id']} — {r['canonical_patch_id']} ({r['regiao']})**")
        # Show first 200 chars of visual notas
