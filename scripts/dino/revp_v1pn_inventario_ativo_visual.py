...
from revp_v1pn_v1pt_execucao_comum import (
...
DOC = _p("REVP_V1PN_DOC", DOCS / "revp_v1pn_inventario_ativo_visual.md")
...
INV_FIELDS = [
    ...
    "file_size_bytes", "patch_id", "alias", "regiao",
    "asset_role_hint", "is_local_only", "is_fixture_or_synthetic",
    "eligible_for_embedding_queue", "blocked_reason", "notas",
]
...
def _scan_dir(d: Path, root: Path) -> list[dict[str, Any]]:
    ...
    for path in sorted(d.rglob("*")):
        ...
        try:
            ...
            size = 0
        name = path.name
        patch, alias, regiao = normalize_patch_from_name(name)
...
def run() -> None:
    ...
    write_doc(DOC, "v1pn — Patch Visual Asset Inventory", [
        ...
        "Nenhum rotulo, target ou ground truth criado.",
        f"## Resultado",
