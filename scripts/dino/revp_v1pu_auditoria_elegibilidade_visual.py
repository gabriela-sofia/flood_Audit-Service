...
DOC = _p("REVP_V1PU_DOC", DOCS / "revp_v1pu_auditoria_elegibilidade_visual.md")
...
def _audit_from_v1fu() -> list[dict[str, Any]]:
    ...
    for i, r in enumerate(read_v1fu_manifest(), 1):
        ...
        pid, alias, regiao = infer_patch_from_path(patch_raw or asset_ref)
        if patch_raw:
            pid = patch_raw.strip().upper()
            alias = pid
        if region_raw:
            regiao = normalize_region(region_raw)
        ...
        # Respect source rotulo/target declarations
        ...
        elig, elig_reason, blocked = classify_dino_eligibility(
            pid, regiao, vtype, conf, fixture, has_label
        )
        ...
        rows.append({
            ...
            "notas": f"source=v1fu protocol_c={'yes' if pid in v1oz_patches else 'no'}",
        })
    return rows
...
def _audit_from_v1fm() -> list[dict[str, Any]]:
    ...
    for i, r in enumerate(read_v1fm_designation(), 1):
        ...
        elig, elig_reason, blocked = classify_dino_eligibility(pid, regiao, vtype, conf, fixture)
        rows.append({
            ...
            "can_train_model": "false",
            "target_created": "false",
            "notas": f"source=v1fm designation={desg}",
        })
    return rows
