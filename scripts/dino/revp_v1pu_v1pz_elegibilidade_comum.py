...
def infer_patch_from_path(path_str: str) -> tuple[str, str, str]:
    ...
    if m:
        ...
        regiao = _REGION_PREFIX.get(prefix, normalize_region(prefix))
        return (pid, pid, regiao)
    # Try raw
    ...
    if m:
        ...
        regiao = _REGION_RAW.get(reg_raw, reg_raw.upper())
        return (pid, f"{prefix.lower()}_{num}", regiao)
    # Region hint only
    m = REGION_FROM_PATH_RE.search(s)
    if m:
        regiao = normalize_region(m.group(1))
        return ("UNKNOWN_PATCH", s.split("/")[-1], regiao)
