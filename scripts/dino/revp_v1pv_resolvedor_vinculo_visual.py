...
DOC = _p("REVP_V1PV_DOC", DOCS / "revp_v1pv_resolvedor_vinculo_visual.md")

REG_FIELDS = [
    "linkage_id", "visual_asset_id", "patch_id", "alias", "regiao",
    ...
    "can_train_model", "target_created", "blocked_reason", "notas",
]
...
def run() -> None:
    ...
    for r in rows:
        if r["eligible_for_dino_review"] == "true":
            regions[r["regiao"]] = regions.get(r["regiao"], 0) + 1
    summary = [
    ...
    write_doc(DOC, "v1pv — Patch Visual Linkage Resolver", [
        "## Objetivo",
        "Ligar assets visuais a patch_id/alias/regiao via manifests. "
