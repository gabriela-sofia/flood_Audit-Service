...
DOC = _p("REVP_V1PZ_DOC", DOCS / "revp_v1pz_pacote_elegibilidade_visual.md")

MANIFEST_FIELDS = ["artifact_id", "stage", "filename", "rows", "header_present", "papel"]
...
def _header(fname: str) -> bool:
    from revp_v1pg_v1pm_representacao_comum import read_csv_header
...
def build_manifest() -> list[dict[str, Any]]:
    return [{
        ...
        "rows": _count(fname), "header_present": str(_header(fname)).lower(), "papel": papel,
    } for i, (stage, fname, papel) in enumerate(ARTIFACTS, 1)]
...
def run() -> None:
    ...
    for rotulo, rows in (("v1pz_manifest", manifest), ("v1pz_summary", summary)):
