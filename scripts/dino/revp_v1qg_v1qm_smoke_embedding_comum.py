...
def read_embedding_rows(path: Path) -> list[dict[str, Any]]:
    """Lê o CSV do variavel store e extrai o vetor de embedding de cada linha.
...
def pca_2d_review(vectors: list[list[float]]) -> tuple[list[tuple[float, float]],
    ...
    try:
        ...
        modelo = PCA(n_components=k, svd_solver="full", random_state=0)
        proj = modelo.fit_transform(x)
        evr = list(modelo.explained_variance_ratio_)
...
def normalize_identity(row: dict[str, str]) -> tuple[str, str, str]:
    ...
    regiao = normalize_region(row.get("regiao", ""))
    return (patch, alias, regiao)
