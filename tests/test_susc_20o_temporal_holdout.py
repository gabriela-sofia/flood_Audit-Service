...
import pipeline_v20o_holdout_temporal_curitiba as p20o  # noqa: E402
...
def _fake_dated(n_pos: int, n_neg: int, year: int, feats: list[str], seed: int) -> pd.DataFrame:
    ...
    d["data_evento"] = pd.to_datetime(f"{year}-06-15")
    d["e_unidade_duplicada"] = False
    return d
...
def test_temporal_holdout_auc_no_leakage_scaler_fit_on_train_only():
    ...
    ajustado no treino (dados sinteticos linearmente separaveis por uma variavel)."""
    ...
    assert result["n_treino"] == n_tr
    assert result["n_teste"] == n_te
    assert 0.0 <= result["auc_holdout"] <= 1.0
    # sinal linearmente separavel por uma variavel -> AUC deve ficar bem acima do acaso
    assert result["auc_holdout"] > 0.8
...
def test_firth_on_train_uses_only_train_rows():
    ...
    assert report["n_pos_train"] + report["n_neg_train"] == report["n_treino"]
    assert report["n_treino"] <= len(d)
    assert set(coefs["variavel"]) == set(p20o.FEATURE_COLS_PRIMARY)


def test_split_excludes_2026_from_train_by_construction():
    """O script principal filtra unidades['ano'].isin(TEST_YEARS) pro teste e o resto pro
