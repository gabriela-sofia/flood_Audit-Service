...
ablacao terreno/chuva usa os mesmos subconjuntos de variavel do resto do projeto (nenhuma
variavel nova, elevation_m continua fora); walk-forward nunca deixa um ano futuro treinar um
...
import pipeline_v20q_diagnosticos_temporais_curitiba as p20q  # noqa: E402
...
def test_bootstrap_ci_resamples_test_only_classifier_fit_once():
    """Sinal linearmente separavel por uma variavel -- o CI bootstrap tem que ficar concentrado
    ...
    train["rotulo"] = (train["rain_decay_index_api_chirps"] > 0).astype(int)
    test = pd.DataFrame({f: rng.normal(size=n_te) for f in FEATS})
    test["rotulo"] = (test["rain_decay_index_api_chirps"] > 0).astype(int)

    result = p20q.bootstrap_holdout_auc_ci(train, test, FEATS, n_boot=500, seed=1)
    assert result["n_teste"] == n_te
...
def test_ablation_terrain_vs_rain_returns_three_rows_correct_feature_counts():
    ...
    assert set(out["grupo_variaveis"]) == {"terreno_so", "chuva_so", "completo_5feat"}
    row = out.set_index("grupo_variaveis")
    assert row.loc["terreno_so", "n_variaveis"] == 3
    assert row.loc["chuva_so", "n_variaveis"] == 2
    assert row.loc["completo_5feat", "n_variaveis"] == 5
...
def test_walk_forward_cutoffs_train_always_strictly_before_test_year():
    ...
    for _, r in out.iterrows():
        anos_treino = [int(y) for y in r["anos_treino"].split(",")]
        assert all(y < r["ano_teste"] for y in anos_treino)
        assert r["ano_teste"] not in anos_treino
...
def test_seasonal_matched_holdout_restricts_train_and_test_to_same_months():
    ...
    d = pd.concat(rows, ignore_index=True)
    out = p20q.seasonal_matched_holdout(d, FEATS, ano_teste=2026,
                                         anos_treino=(2023, 2024, 2025), months=(1, 7))
    # so mes=2 cai na janela 1-7; treino deve ter 3 anos x 90 unidades, teste 1 ano x 90
    assert out["n_treino"] + out["n_teste"] <= 4 * 90  # nunca inclui mes=9 (fora da janela)
    assert out["n_teste"] <= 90
...
def test_coef_stability_by_year_marks_epv_failure_when_too_few_negatives():
    ...
    assert out["coef_padronizado"].isna().all()
...
def test_metadata_composition_stability_reports_one_row_per_year():
    ...
    for ano in [2023, 2024, 2025, 2026]:
        ...
        d = pd.DataFrame({
            "rotulo": [1] * 25 + [0] * 15,
            "nivel_confianca": (["strong"] * 23 + ["medium"] * 2) + ["strong"] * 15,
            "assunto": (["Drenagem"] * 20 + ["Proteção ao cidadão - GM"] * 5) + [None] * 15,
            "registro_fonte": ["reg_pos.csv"] * 25 + ["reg_neg.csv"] * 15,
            "tipo_fonte_negativo": [None] * 25 + ["tipo_a"] * 15,
            "n_grupo_duplicata": [1] * n,
            "rain_status": ["OK"] * n,
    ...
    assert sorted(out["ano"]) == [2023, 2024, 2025, 2026]
