...
import pipeline_v20v_decomposicao_gbm_varredura_curitiba as p20v  # noqa: E402
...
def test_partial_dependence_covers_all_five_causal_features_no_elevation():
    ...
    for feat, info in out.items():
        assert info["grade_min"] <= info["grade_max"]
        assert info["direcao"] in {"crescente", "decrescente"}
...
def test_partial_dependence_grid_is_in_original_feature_scale_not_standardized():
    ...
    como algo tipo -2..2 -- checamos que a grade reflete a escala real da variavel."""
    d = _fake(400, seed=2)
    out = p20v.gbm_partial_dependence(d, FEATS)
    ...
    span = out["hand_m_dinf"]["grade_max"] - out["hand_m_dinf"]["grade_min"]
    assert span > 6
...
def test_model_sweep_covers_expected_model_classes():
    ...
    assert set(out["modelo"]) == expected
